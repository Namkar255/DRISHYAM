...
| Temporal analytics and incident-window pattern rules | **Done** — contact placed before/during/after the declared incident window, plus `PRE_INCIDENT_COMMUNICATION`, `COMMUNICATION_BURST`, `RECURRING_VEHICLE_AT_LOCATION` and `BRIDGE_ENTITY`. A case with no declared window says so rather than choosing one. Cross-case alerting is deliberately absent: it would disclose the existence of another case |
